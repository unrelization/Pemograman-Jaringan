...
def create_db():
    ...
    if not env.db_name:
        ...
        db_name = env.db_name
    run('echo "CREATE DATABASE %s default character set utf8 collate utf8_unicode_ci;" | \
         mysql --batch --user=%s --password=%s --host=%s' % (db_name, env.mysqluser, env.mysqlpassword, env.mysqlhost), pty=True)
...
def empty_db():
    ...
    (mysqldump -u%s -p%s --add-drop-table --no-data %s | grep ^DROP);
